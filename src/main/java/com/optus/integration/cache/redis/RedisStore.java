package com.optus.integration.cache.redis;
...
public class RedisStore <T extends Serializable> implements ObjectStore<T>, MuleContextAware {
	...
	public void setTimeToLive() {
			System.out.println("this.timeToLive : " + this.timeToLive);
			this.cacheManager.setDefaultExpiration(this.timeToLive);
	}*/
	...
	public synchronized void store(Serializable key, T value) throws ObjectStoreException {
		...
		/*Object objKey = (Object)key.toString();
		RedisCacheKey cacheKey = new RedisCacheKey(objKey);
		RedisCacheElement element = new RedisCacheElement(cacheKey, data);
		element.setTimeToLive(100);
		
		System.out.println("element get : " + element.get().toString());
		System.out.println("element : " + element.getKey());
		System.out.println("this.cache : " + this.cache.get(key.toString()));
		System.out.println("cacheKey 0 : " + cacheKey.toString());
		this.cache.put(element);
		System.out.println("cacheKey : " + cacheKey.toString());*/
	
		if(data != null && data.size() > 0){
			
			Map tokenData = (Map)data.get(0);
			
			if (tokenData != null) {				
				Integer tokenExpiry  = (Integer) tokenData.get("expires_in");
				
				//Jedis jedis = null;

				/*//try {
			        jedis = this.jedisPool.getResource();
			        Long time = jedis.expire(key.toString(), tokenExpiry);
			        jedis.close();
			    } catch (Exception je) {
			        throw je;
			    }*/
				
				/*//this.cacheManager.setDefaultExpiration(100);				
				Jedis jedis = new Jedis("localhost", 6380, 10000);
				Long time = jedis.expire(key.toString(), tokenExpiry);
				jedis.close();*/
			}
		}
	...
	public synchronized T retrieve(Serializable key) throws ObjectStoreException {
		...
		if(token != null && token.size() > 0){
			...
			if (tokenData != null) {
				...
				/*Object objKey = (Object)key.toString();
				RedisCacheKey cacheKey = new RedisCacheKey(objKey);
				RedisCacheElement element = new RedisCacheElement(cacheKey, token);
				element.expireAfter(tokenExpiry);
				System.out.println("TTL : " + element.getTimeToLive());*/
